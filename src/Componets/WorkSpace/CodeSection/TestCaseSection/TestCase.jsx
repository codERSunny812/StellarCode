  ...
  return (
    <div className='h-[calc(100vh-400px)] text-white bg-[#282828] mr-4 overflow-hidden'>
      ...
      <div className=" mx-4 my-1 px-4 py-1">
        {
          examples.map((example)=>(
            <button
              ...
              className={`mx-2 focus:bg-[#3E3D3D] hover:bg-[#3E3D3D] px-3 py-1 rounded-lg ${selectedCase.id === example.id ? "bg-[#3E3D3D]" : ""
                }`}
            >
              Case {example.id}
            </button>
      ...
      <div className=" my-1 mx-2 px-4 py-1">
          ...
          <p className="px-3 py-2 bg-[#3E3D3D] rounded-lg  font-bold w-full"> {selectedCase.inputText} </p>
       
       
    
        
          <h1 className="my-2 mx-2 capitalize">output:</h1>
          <p className="px-3 py-2 w-full bg-[#3E3D3D] rounded-lg  font-bold"> {selectedCase.outputText}</p>
